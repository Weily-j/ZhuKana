...
### 4. 字級比例：主副 ≥ 2.6:1（教學導向）

主字（注音教學標籤，如 ㄎ／母音）1.625rem weight 500；副字（假名行名，如 か行）0.625rem weight 400 opacity 0.55。
比例 ≥ 2.5，遠看主導的是注音提示（ZhuKana 差異化），近看仍清楚看到假名行名。

**路線釐清**：雖然 PM 原意是「iOS 擬真＋注音 subtitle」，但經過 uiux review 後確認目前實作（主=注音、副=假名）更符合 ZhuKana 作為「注音優先教學工具」的核心定位，故 spec 從現況走，不對調主副。

**替代方案（已棄）**：主副同行並列（あ ㄚ）——放棄，佔寬過大、對齊困難；主=假名、副=注音——放棄，失去教學價值。
